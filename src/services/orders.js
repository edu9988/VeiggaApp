...
const baseURL = `https://api.tiendanube.com/v1/${user_id}/draft_orders`
const config = {
  headers: {
  ...
  while(true){
    try{
      ...
      if( link === undefined || link.indexOf( 'rel="next"' ) === -1 ){
        ...
        console.log('order next url:', url)
      }
    }
    catch(error){
...
const post = async (newOrder) => {
  try{
    return await axios.post(baseURL, newOrder, config)
  }
  catch(error){
...
const put = async (id, updatedOrder) => {
  try{
    return await axios.put(`${baseURL}/${id}`, updatedOrder, config)
  }
  catch(error){
